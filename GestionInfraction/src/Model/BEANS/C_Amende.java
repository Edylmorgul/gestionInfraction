...
public class C_Amende implements Serializable{
	...
	public C_Amende(int id, C_TypeVehicule vehi, C_Policier pol, C_Contrevenant con, String immatriculation, Date instant, String ville, Double montant, String commentaire, int valide, int envoye) {
		this.id = id;
		this.vehi = vehi;
		this.pol = pol;
		this.con = con;
	...
	// Fonction pour calculer montant total amende par rapport aux infractions
	public void calculerTotalAmende() {
		// Calcul des montants des infractions
		for(C_TypeInfraction infra : listeInfra) {
			this.montant += infra.getMontant();
		}
		
		// Calcul pourcentage de frais
		calculerPourcentageFraisAmende();
		
		// Ajout du montant frais de dossier
		this.montant += Global.fraisDossier;
	}
	
	// Fonction pour calculer les frais supplémentaires par nombre d'infractions
	private void calculerPourcentageFraisAmende() {
		// Reception total d'infraction
		int cpt = listeInfra.size();
		double frais = 0;
		
		switch(cpt) {
		// 5%
		case 1:
			frais = this.montant * 0.05;
			this.montant += frais;
			break;
		// 20%
		case 2:
			frais = this.montant * 0.20;
			this.montant += frais;;
			break;
		// 30%
		case 3:
			frais = this.montant * 0.30;
			this.montant += frais;
			break;
		// 50%
		default:
			frais = this.montant *0.50;
			this.montant += frais;
			break;
		}
	}
